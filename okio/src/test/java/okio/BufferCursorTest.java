...
public final class BufferCursorTest {
  ...
  @Test public void enlargeByManySegments() throws Exception {
    ...
    cursor.acquireReadOnly(buffer);
    try {
      cursor.resize(originalSize + 1_000_000);
      cursor.seek(originalSize);
      do {
  ...
  @Test public void shrinkByManySegments() throws Exception {
    ...
    BufferCursor cursor = new BufferCursor();
    cursor.acquireReadOnly(toShrink);
    try {
      cursor.resize(originalSize);
    } finally {
      cursor.release();
    }
  ...
  @Test public void resizeToSameSizeRetainsOffset() throws Exception {
    ...
    cursor.acquireReadOnly(buffer);
    try {
      cursor.seek(cursor.size / 2);
      assertEquals(originalSize, cursor.size);
      long offsetBefore = cursor.offset;
      byte[] dataBefore = cursor.data;
      int posBefore = cursor.pos;
      int limitBefore = cursor.limit;
      cursor.resize(originalSize);
      assertEquals(originalSize, cursor.size);
      assertEquals(offsetBefore, cursor.offset);
      assertEquals(dataBefore, cursor.data);
      assertEquals(posBefore, cursor.pos);
      assertEquals(limitBefore, cursor.limit);
    } finally {
      cursor.release();
    }
  }
  ...
  @Test public void enlargeRetainsOffset() throws Exception {
    ...
    try {
      ...
      assertEquals(originalSize, cursor.size);
      long offsetBefore = cursor.offset;
      cursor.resize(originalSize * 2);
      assertEquals(originalSize * 2, cursor.size);
      assertEquals(offsetBefore, cursor.offset);
    } finally {
      cursor.release();
    }
